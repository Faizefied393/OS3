...
bufferRequest buffer[MAX_REQUESTS];
int buf_size = 0;
int scheduling_algo = 0; // 0 = FIFO, 1 = SFF, 2 = Random
...
void* thread_request_serve_static(void* arg) {
    while (1) {
        ...
        bufferRequest req = buffer[index];
...
//
// Initial handling of the request (producer logic)
//
void request_handle(int fd) {
    ...
    if (is_static) {
        ...
        bufferRequest req = { fd, strdup(filename), sbuf.st_size };

        pthread_mutex_lock(&buffer_lock);
        while (buf_size == MAX_REQUESTS) {
